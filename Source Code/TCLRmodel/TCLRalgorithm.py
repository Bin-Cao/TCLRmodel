"""
    ...
    Reference : Domain knowledge guided interpretive machine learning ——  Formula discovery for the oxidation behavior of Ferritic-Martensitic steels in supercritical water. Bin Cao et al., 2022, journal paper.
"""
...
def createTree(dataSet, correlation='PearsonR(+)', minsize = 3, threshold = 0.95, mininc = 0.01):
    ...
    if correlation == 'PearsonR(+)':
        ...
        if splitSuccess:
            node.lc, node.rc = createTree(lc, correlation, minsize, threshold, mininc), createTree(rc, correlation, minsize, threshold, mininc)
...
def render(label, node, dot, feats):
    ...
    if node.slope == None:
        dot.node(label, "#=" + str(node.size) + " , ρ=" + str(round(node.R, 3)))
    else:
        dot.node(label, "#=" + str(node.size) + " , ρ=" + str(round(node.R, 3)) + ' , slope=' + str(round(node.slope, 3)))
...
def start(filePath, correlation, minsize, threshold, mininc):
    ...
    root = createTree(csvData, correlation, minsize, threshold, mininc)

    dot = Digraph(comment='Result of TCLR')
    render('A', root, dot, feats)
    dot.render('Result of TCLR {year}.{month}.{day}-{hour}.{minute}'.format(year=namey, month=nameM, day=named, hour=nameh, minute=namem))
